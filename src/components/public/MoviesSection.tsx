...
const MovieCard = ({ movie, index, onClick }: { movie: FeaturedMovie; index: number; onClick: () => void }) => {
  const [imgErr, setImgErr] = useState(false);
  const [backdropErr, setBackdropErr] = useState(false);
  const hasBackdrop = movie.backdrop_url && !backdropErr;
  const hasPoster = movie.poster_url && !imgErr;
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-2xl border border-border/20 bg-card premium-shadow-sm aspect-[2/3] transition-all duration-300 group-hover:scale-[1.03] group-hover:premium-shadow group-hover:border-primary/20">
        ...
        {/* Main poster */}
        {hasPoster ? (
          <img src={movie.poster_url!} alt={movie.title} className="relative w-full h-full object-cover" loading="lazy" onError={() => setImgErr(true)} />
        ) : (
        ...
        {movie.rating && (
          <div className="absolute top-2.5 right-2.5 flex items-center gap-1 rounded-lg bg-amber-500/90 backdrop-blur-sm px-2 py-1 shadow-md">
            ...
            <span className="text-[11px] font-bold text-white">{Number(movie.rating).toFixed(1)}</span>
          </div>
        )}
        <div className="absolute bottom-0 left-0 right-0 p-3.5 space-y-1.5">
          <p className="text-sm font-bold text-foreground leading-tight line-clamp-2 drop-shadow-lg">{movie.title}</p>
          <div className="flex items-center gap-2 flex-wrap">
            {movie.year && <span className="text-[11px] text-foreground/50 font-medium">{movie.year}</span>}
            {movie.genre && (
...
export const MoviesSection = () => {
  ...
  if (isLoading) {
    return (
      <div className="space-y-5" id="filmes">
        <SectionHeader />
    ...
    return (
      <div className="space-y-5" id="filmes">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">Nenhum filme em destaque</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-5" id="filmes">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5">
        {movies.map((movie, idx) => (
          <MovieCard key={movie.id} movie={movie} index={idx} onClick={() => setSelectedMovie(movie)} />
        ))}
      </div>
      <ContentDetailSheet
        open={!!selectedMovie}
        onClose={() => setSelectedMovie(null)}
        item={selectedMovie ? { ...selectedMovie, content_type: "movie" } : null}
      />
    </div>
  );
};
...
const SectionHeader = ({ count }: { count?: number }) => (
  <div className="flex items-center justify-between">
    <div className="flex items-center gap-2.5">
      ...
      <h2 className="font-display text-base sm:text-lg font-bold text-foreground tracking-tight">Filmes em Destaque</h2>
    </div>
    {count !== undefined && (
