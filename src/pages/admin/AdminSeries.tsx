...
const AdminSeries = () => {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Buscar Séries (TMDB)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <Tabs defaultValue="search">
            <TabsList>
              <TabsTrigger value="search">Buscar</TabsTrigger>
              <TabsTrigger value="popular" onClick={loadPopular}>Populares</TabsTrigger>
            </TabsList>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {results.map((r) => (
                <div key={r.id} className="group relative rounded-lg border border-border/30 bg-card overflow-hidden">
                  ...
                  <div className="p-2">
                    <p className="text-xs font-medium truncate">{r.name || r.title}</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Séries Adicionadas ({series?.length || 0})</CardTitle>
        </CardHeader>
        <CardContent>
          {!series || series.length === 0 ? (
            <p className="text-sm text-muted-foreground">Nenhuma série adicionada</p>
          ) : (
            <div className="space-y-2">
              {series.map((s) => (
                <div key={s.id} className="flex items-center gap-3 rounded-lg border border-border/50 bg-card p-2">
                  {s.poster_url ? (
                    <img src={s.poster_url} alt={s.title} className="h-14 w-10 rounded object-cover" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">{s.year} • ⭐ {s.rating}</p>
                  </div>
                  <Switch checked={s.active} onCheckedChange={(v) => toggleSeries.mutate({ id: s.id, active: v })} />
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => { if (confirm("Remover série?")) deleteSeries.mutate(s.id); }}>
