...
import { Trash2, Upload, ArrowUp, ArrowDown, Loader2, Image, ClipboardPaste, Clock, PowerOff } from "lucide-react";
...
const PasteZone = ({ onImagePasted, uploading }: { onImagePasted: (file: File) => void; uploading: boolean }) => {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (file && file.type.startsWith("image/")) onImagePasted(file);
  }, [onImagePasted]);
...
const AdminBanners = () => {
  ...
  // Use global indices for reordering
  ...
  const handleDeactivateAll = async () => {
    ...
    if (!confirm(`Desativar ${activeOnes.length} banner(s) ativo(s) em ${CATEGORY_LABELS[selectedCategory]}?`)) return;
    try {
      await Promise.all(activeOnes.map((b) => updateBanner.mutateAsync({ id: b.id, active: false })));
      toast.success(`${activeOnes.length} banner(s) desativado(s)`);
    } catch { toast.error("Erro ao desativar banners"); }
  };
            ...
            <div className="p-4 border-b border-white/[0.06]">
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  ...
                  {activeBanners > 0 && (
                    <Button variant="ghost" size="sm" onClick={handleDeactivateAll} className="text-[11px] text-destructive hover:bg-destructive/10 gap-1.5">
              ...
              {isLoading ? (
                ...
                <div className="space-y-5">
                  {(() => {
                    ...
                    // Build a flat list of global indices per banner id
                                ...
                                <div className="p-3 space-y-1">
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-1">
                                      <Button size="icon" variant="ghost" className="h-9 w-9 rounded-lg hover:bg-white/[0.06]" disabled={gIdx === 0} onClick={() => moveBanner(banner.id, "up")}><ArrowUp className="h-4 w-4" /></Button>
                                      <Button size="icon" variant="ghost" className="h-9 w-9 rounded-lg hover:bg-white/[0.06]" disabled={gIdx === banners.length - 1} onClick={() => moveBanner(banner.id, "down")}><ArrowDown className="h-4 w-4" /></Button>
